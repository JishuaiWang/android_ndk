...
import com.example.kunrui.kunrui.R;
...
public class RadarView extends View {
    ...
    private double powerSize = 0;//点距离圆心长度百分比
    private double angle = 0;//园角度
    //默认的主题颜色
    private int DEFAULT_COLOR = Color.parseColor("#91D7F4");
    //水滴颜色
    private int RED_COLOR = Color.parseColor("#FFFF0019");
    ...
    public void Init(double powerSize_t, double angle_t, int mRaindropNum_t){
        mRaindropNum = mRaindropNum_t;
        powerSize = powerSize_t;
        angle = angle_t;
    }
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mRaindropColor = mTypedArray.getColor(R.styleable.RadarView_raindropColor, RED_COLOR);
    ...
    private void generateRaindrop(int cx, int cy, int radius) {
        ...
        if (mRaindrops.size() < mRaindropNum) {
            ...
            if (probability) {
                int x = 0;
                int y = 0;
                int xOffset = (int)((radius-20) * Math.cos(angle) * powerSize);
                int yOffset = (int)((radius-20) * Math.sin(angle) * powerSize);

                x = cx + xOffset;
                y = cy + yOffset;
//                System.out.println("x:"+x+"\txOffset:"+xOffset+"\ty:"+y+"\tyOffset:"+yOffset);
